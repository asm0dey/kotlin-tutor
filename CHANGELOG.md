...
The Kotlin API design guidance lands as first-class plugin content — new `alwaysApply` rules and a skill — rather than a pointer to an external document, so the agent carries the guidance itself, not a link to go read.

### Added

- **Five new idiom rules**, distilling the broadly-applicable Kotlin API design principles into the plugin's always-on voice (they apply to all Kotlin, not just published libraries):
  - **K-8 `avoid-boolean-arguments`** — a `Boolean` parameter hides its meaning at the call site (`copy(true)` — overwrite what?); split into named functions (`map` / `mapNotNull`) or an `enum`, and require named arguments when a flag is unavoidable
  - **K-9 `read-only-collections`** — signatures take and return `List` / `Set` / `Map`, not the `Mutable` variants; never hand out the live internal mutable collection; `Array` out of public signatures (`vararg` excepted)
  - **K-10 `require-check-validation`** — `require()` for argument validation (`IllegalArgumentException`), `check()` for instance-state validation (`IllegalStateException`); put the offending value in the message, never secrets
  - **K-11 `sealed-for-closed-hierarchies`** — `sealed` over `open` when the set of subtypes is fixed, for exhaustive `when` with no `else`; don't add an `else` that defeats the exhaustiveness check
  - **K-12 `meaningful-tostring`** — stateful non-`data` classes override `toString()` to render state for logs and debuggers (the default prints a useless identity hash), never leaking secrets; complements the generated `toString()` a `data class` already provides (K-3)
- **New skill `kotlin-api-review`** — reviews a published API surface for the concerns that only matter once an API ships, covering all the themes from the established Kotlin library-design guidance: the six mental-complexity attributes (simplicity, readability, consistency, predictability, debuggability, testability), plus backward compatibility (binary/source/behavioral distinction, explicit return types + Explicit API mode, manual overloads over default args, regular classes over data classes on public surface, `@PublishedApi`-is-public, return-type widening/narrowing, deprecation cycles, Binary Compatibility Validator, `@RequiresOptIn` with propagation), multiplatform (broadest-source-set placement, common-code design, cross-platform behaviour consistency, `expect`/`actual`, target coverage), and documentation (KDoc on every entry point, lambda exception/concurrency semantics, examples). Self-contained — the guidance is embedded, not linked out. Intent-discovered, so it carries **zero per-turn context cost**: it loads only when the user is actually designing or reviewing a public API. Skill count grows from three to four
- Rule count grows from thirteen to eighteen; idiom rules from seven to twelve. README rules table, intro, Rules header, and Conventions section updated
